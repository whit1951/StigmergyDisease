#' MoveLoopFunction.R
#' Adapting StigmergyLoopFunction.R so that there are no directional restrictions on movemen
#' in response to scent cues, i.e., using function `new.cell` instead of `new.cell.directed` for movement
#' @author Lauren White
#' @date May 17, 2019
...
MoveLoop<-function(maxT, nsim, lsize, n.initial, inf_prob, rec_rate, dur_scent, initial_load, scent_decay, inf_decay){
  ...
  for(count in 1:nsim){
    ...
    for(t in 2:maxT){
      ...
      inds<-new.cell(possible_loc,inds, lsize)
  ...
  #Record output
  write.csv(summary,file=paste(filename,"undir_summary.csv", sep="_"))
  write.csv(infected,file=paste(filename,"undir_infected.csv", sep="_"))
  print(filename)
}
...
# tic=Sys.time()
# MoveLoop(maxT=100, nsim=2, lsize=50, n.initial=100, inf_prob=0.1, rec_rate=0.1, dur_scent=10, initial_load=10, scent_decay=0.1, inf_decay=0.1)
