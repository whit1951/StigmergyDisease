...
#' @param scent_load - duration of scent mark time steps
#' @param infected - boolean, if TRUE create infection landscape

createland<- function(lsize, inds, scent_load, infected)
{
  landscape<- array(0, dim= c(lsize, lsize, nrow(inds))) #y, x, one layer for each individual
  if(infected==FALSE){
  for(i in 1:nrow(inds))
  {
    landscape[inds$yloc[i], inds$xloc[i], i]<- scent_load #row, column, layer
  }
  }
  if(infected==TRUE){
    ...
    {
      landscape[inds$yloc[i], inds$xloc[i], infID[i]]<- scent_load #row, column, layer
    }
  }
  return(landscape)
}

#test<-createland(lsize=5, N=5, inds=inds, scent_load=10)
...
#' @author James Forester
#' @export
get.dist.weight <-function(celldist, min.dist=0.75, dist.weight.fun=NULL,...){
...
new.cell<-function(possible_loc, inds, lsize, dist_weights){
  for(i in 1:nrow(inds)){
    inds$vec[i]<-sample(possible_loc[i,], 1, prob=dist_weights)
...
#' @param possible_loc - possible movement values generated from get.neighbors function
#' @param inds - dataframe of individual traits
#' @param prob_mat - probability matrix created with the `create.prob` function
#' @param lsize - size of landscape (1 dimension)
#' @param durscent -strength of initial scent deposit
...
#' @date May 7, 2019
#' 
infect_indirect<-function(inds, nS, nI, lxy, inf_landscape){
...
#'@variable dist_weights - vector of distance weighting produced by `get.dist.weight` function
create.prob<-function(dist_weights){
  ...
  #include distance weights and normalize probabilities
  #prob_mat<-sweep(prob_mat, MARGIN=2, dist_weights, `*`)
  for(i in 1:nrow(prob_mat))
  {
    prob_mat[i,]<-prob_mat[i,]*dist_weights/sum(prob_mat[i,]*dist_weights)
  }
  return(prob_mat)
}
